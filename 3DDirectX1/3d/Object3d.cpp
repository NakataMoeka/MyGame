...
void Object3d::transformParent(Object3d* obj)
{
	//どうやるんですか
	//Update()と同じような感じ
	g_Trans = obj->g_Trans + XMMatrixTranslation(obj->position.x, obj->position.y, obj->position.z);
	//回転角度
	g_Rot = XMMatrixIdentity();
	g_Rot += XMMatrixRotationZ(XMConvertToRadians(obj->rotation.z));	//Z軸まわりに45度回転
	g_Rot += XMMatrixRotationX(XMConvertToRadians(obj->rotation.x));
	g_Rot += XMMatrixRotationY(XMConvertToRadians(obj->rotation.y));
